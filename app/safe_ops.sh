# איפוס וכתיבה
rm -f /app/safe_ops.sh
install -m 755 /dev/stdin /app/safe_ops.sh <<'BASH'
#!/usr/bin/env bash
# safe_ops.sh — curl+openssl; POST חתום (ts.nonce.body), GET עם Bearer
...
# ===== בסיס =====
healthz(){ _sg "/readyz" && echo OK; _sg "/health"; }
pending(){  _sg "/ops/ui/pending"; }
digest(){   _sg "/ops/digest/expired?hours=${1:-6}"; }

# ===== ניהול פוזיציה (קיימים בשרת) =====
status(){   _sg "/position-ops/status?symbol=${1:?SYMBOL}"; }
...
type -a healthz status be trail manage_once >/dev/null && echo "functions ok"
