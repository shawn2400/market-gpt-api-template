...
# סמלים פופולריים – אפשר להגדיל או לשנות
POPULAR_SYMBOLS = [
...
async def analyze_symbol(symbol: str, interval: str = "15m", limit: int = 100):
    try:
        ...
        direction = "LONG" if last["rsi"] < 35 and last["adx"] > 20 and last["close"] > last["ema_21"] \
...
async def scan_all_futures(symbols: list = None, interval: str = "15m", limit: int = 100, min_quality: int = 5):
    """
    סורק את כל הסמלים ומחזיר רק את החזקים ביותר (quality_score >= min_quality)
    """
    symbols = symbols or POPULAR_SYMBOLS
    tasks = [analyze_symbol(s, interval, limit) for s in symbols]
    results = await asyncio.gather(*tasks)
    # סינון: רק תוצאות טובות, ממויין לפי quality_score (גבוה->נמוך), ואז נפח
    filtered = [
