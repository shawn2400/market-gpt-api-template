...
- **Tier 3 Quality Threshold Fix (Nov 19)**: Fixed inverted logic - Tier 3 (Weak/CHOPPY market) now uses min_quality=4.5 (down from 6.0) and WAIT strategy uses 5.5 (down from 6.5). CHOPPY markets now correctly have LOWER thresholds, enabling trades with scores 5.5-6.4 to pass. Restores 100% dynamic autonomous trading across all market conditions.
- **BanShield v2.2 Complete Position Management Priority (Nov 19)**: futures_klines AND futures_get_open_orders upgraded from LOW/NORMAL→CRITICAL priority permanently. Unblocks Trailing SL/TP Extension calculations AND SL/TP order verification in RED zone (42+ RPM), ensuring position management never fails due to rate limiting. Fixes "_update_trailing_sl() not executing" bug and eliminates "Blocked NORMAL call: futures_get_open_orders" errors. Complete unblocking of Fills Watcher and Position Monitor during high API load.
- **Debug Route Import Fix (Nov 19)**: Fixed `routes/debug_sig.py` canonical string builder - now correctly uses individual `_canon()` calls for route/nonce/body instead of invalid 4-parameter call. Eliminates "cannot import name '_canon'" warning on startup.
- **Context URL Warning Silenced (Nov 19)**: Changed CONTEXT_URL fallback log level from WARNING → DEBUG. This is normal fallback behavior when Context API unavailable, not an error requiring user attention.
...
- **Order Hygiene Temporary Disable (Nov 19)**: Order hygiene cleanup temporarily disabled in Position Monitor (awaiting module completion). Non-critical feature - core position management unaffected.
- **Multi-Target TP System v2.0 + Dynamic Extension + Trailing SL (Nov 19)**: Complete 3-level Take Profit implementation (TP1/TP2/TP3) with dynamic exit percentages (25/40/35 default, adapts to volatility/regime). **NEW**: Dynamic TP Extension - automatically generates TP4/TP5/TP6 when TP3/TP4 hit, with ATR-based spacing. **NEW**: Dynamic Trailing SL - activates after TP1, moves Stop Loss up as price climbs (2-5% from peak based on volatility), tightens progressively at higher TP levels (80%@TP3, 60%@TP4), synchronized with TP Extension. Telegram notifications for all updates. BinanceSymbolValidator integration ensures precision compliance. ExecutionBot auto-attaches to all new positions, Position Monitor Layer 0 backfills existing positions. Verified working in production.
