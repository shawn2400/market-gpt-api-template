...
- **BanShield v2.2 Position Management Priority (Nov 19)**: futures_klines upgraded from LOW→NORMAL priority, receives CRITICAL boost when positions open. Unblocks Trailing SL/TP Extension calculations in RED zone (42+ RPM), ensuring position management never fails due to rate limiting. Fixes "_update_trailing_sl() not executing" bug.
