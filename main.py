...
# ===== Imports =====
from utils import config as cfg  # noqa
...
# ===== FastAPI =====
app = FastAPI(title="AlgoGPT API", version=APP_VERSION, description="AlgoGPT — מסחר אלגוריתמי בזמן אמתי")
...
ROUTERS: List[str] = [
    ...
    "routes.rpc","routes.market_extra","routes.executor_extra","routes.anchor",
    "routes.grid","routes.debug","routes.indicators","routes.indicators_extra",
    "routes.telegram_webhook","routes.metrics","routes.metrics_extra","routes.precision","routes.alerts",
    ...
    "routes.orderflow","routes.analysis","routes.decision"
]
...
# ===== Health =====
@app.get("/") async def root_status(): return {"ok": True, "status": "ok", "version": APP_VERSION}
@app.get("/health") async def health(): return {"ok": True, "status": "ok", "version": APP_VERSION}
@app.get("/healthz") async def healthz_alias(): return {"status": "ok", "service": "algogpt", "version": APP_VERSION}
@app.get("/readyz")
async def readyz():
