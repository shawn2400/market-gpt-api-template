...
# Routers (שים לב: analytics במקום analysis; decision הוסר זמנית)
for module_path in ("routes.trade", "routes.analytics"):
    try:
...
async def _tg_send(chat_id: int, text: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as cli:
            await cli.post(f"{API_BASE}/sendMessage", data={"chat_id": chat_id, "text": text, "parse_mode": "HTML", "disable_web_page_preview": True})
    except Exception as e:
...
async def telegram_webhook(request: Request, x_telegram_bot_api_secret_token: str | None = Header(default=None)):
    ...
    if cb:
        ...
        if len(parts) == 3 and parts[0] == "CONFIRM":
            ...
            if action == "APPROVE":
                ConfirmStore.approve(cid, approver=approver); await _tg_send(chat_id, "✅ אושר. מפעיל את הטרייד.")
            else:
                ConfirmStore.reject(cid, approver=approver); await _tg_send(chat_id, "❌ בוטל. הטרייד לא יצא לפועל.")
        return {"ok": True}
    msg = update.get("message")
    if msg and str(msg.get("text", "")).strip() == "/ping":
        chat_id = int(msg.get("chat", {}).get("id", 0)); await _tg_send(chat_id, "pong ✅"); return {"ok": True}
    return {"ok": True}

@app.on_event("startup")
async def _startup():
    if not BOT_TOKEN: return
    if os.getenv("TELEGRAM_AUTO_WEBHOOK", "1") not in ("1","true","yes","on"): return
    public_host = os.getenv("PUBLIC_HOST", "").strip()
    if not public_host: return
    try:
        async with httpx.AsyncClient(timeout=10.0) as cli:
            await cli.post(f"{API_BASE}/setWebhook", data={"url": f"{public_host}/telegram/webhook","secret_token": WEBHOOK_SECRET,"drop_pending_updates": "true","max_connections": "40"})
    except Exception as e:
