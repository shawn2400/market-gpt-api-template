...
# Metrics last, so הוא ימדוד את כל המסלול (כולל אימות)
...
DEFAULT_PUBLIC_PATHS = {
    ...
    # alerts ציבורי – האימות בתוך ה־router
...
async def add_server_identity_header(request: Request, call_next):
    # אל תעטוף ב-try/except שמסתיר Response; אם downstream יזרוק – FastAPI יחזיר 500.
...
try:
    ...
    def ws_user_status(): return {"running": False}
    def exec_get_counters(): return {"tick_ewma_ms": None}

@app.get("/status/ws")
async def status_ws(): return {"ok":True, **ws_user_status()}

@app.get("/status/executor")
async def status_executor(): return {"ok":True, **exec_get_counters()}
