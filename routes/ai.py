...
def _mk_anchor_dict(anchor: AnchorDecision) -> Dict[str, Any]:
    return {"mode_requested": getattr(anchor, "mode_requested", None),
...
async def post_ai_quality(payload: QualityRequest = Body(...), _auth=Depends(require_api_key)) -> QualityResponse:
    anchor = evaluate_anchor(payload.side)
    q = compute_quality(symbol=payload.symbol, side=payload.side,
    ...
    return QualityResponse(quality_score=float(q.get("quality_score", 0.0)),
                           success_pct=float(q.get("success_pct", 0.0)),
                           components=q.get("components") or {}, anchor=_mk_anchor_dict(anchor))

@router.get("/manual-scan", response_model=AiManualScanResponse)
async def get_ai_manual_scan(symbol: str = Query(...), market: str = Query("futures"),
                             interval: str = Query("15m"),
                             bars: int = Query(200, ge=50, le=500),
                             _auth=Depends(require_api_key)) -> AiManualScanResponse:
    ...
    try:
        ...
        return AiManualScanResponse(symbol=sym,
                                    results=AiManualScanItem(symbol=sym, market=market,
                                                             interval=interval,
