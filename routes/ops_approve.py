...
import os, time, hmac, hashlib, json, logging, urllib.parse
...
def _sig_canonical(qs_params: Dict[str, str]) -> str:
    """HMAC over sorted k=v joined with &; excludes 'sig' if present."""
    if not WEBHOOK_SECRET: return ""
    items = {k: v for k, v in qs_params.items() if k != "sig" and v is not None}
    # Keep only known/benign keys to avoid confusion
    allow = {"ticket_id","action","expires","require","version","by"}
    items = {k: items[k] for k in items.keys() if k in allow}
    canon = "&".join(f"{k}={items[k]}" for k in sorted(items.keys()))
...
async def ops_approve(
    ...
    require:  Optional[int] = Query(None, ge=1, le=2),   # optional, for canonical signature
    version:  Optional[str] = Query(None),                # optional, for canonical signature
    # trade params:
    ...
    if ticket_id or action or expires or sig or require or version:
        ...
        # validate signature (accept either legacy OR canonical)
        qs = {
            "ticket_id": ticket_id,
            "action": action,
            "expires": str(expires),
        ...
        event = {
            "id": ticket_id,              # שימור השם 'id' עבור הקורא מהסופרוויזור
...
async def ops_approve_signed(request: Request, x_signature: str = Header(default="", alias="X-Signature")):
    ...
    if budget is None and qty is not None:
        try: budget = float(os.getenv("MIN_NOTIONAL_USDT", "5"))
        except Exception: budget = 5.0
    req_payload: Dict[str, Any] = {
        ...
        "dry_run": False,
        "market": "futures",
        "meta": {"approved_via": "POST /ops/approve/signed", "position_side": position_side, "ticket_id": body.get("ticket_id"), "ts": int(time.time())}
    }
    if qty is not None: req_payload["quantity"] = float(qty)
    if budget is not None: req_payload["budget"] = float(budget)
    result = await _post_grid_trade(req_payload)
    return {"ok": bool(result.get("ok")), "request": req_payload, "result": result}
