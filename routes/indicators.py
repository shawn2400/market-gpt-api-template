...
router = APIRouter(tags=["Indicators"], dependencies=[Depends(require_api_key)])
...
async def get_indicators_sample(request: Request) -> IndicatorsResponse:
    ...
    sample = IndicatorSignal(symbol="BTCUSDT", timeframe="1h",
...
async def get_indicators_symbol(symbol: str = Path(...), timeframe: str = Query("1h"),
                                limit: int = Query(180, ge=50, le=500),
                                request: Request = None) -> IndicatorsResponse:
    if not _rl(request.client.host): raise HTTPException(429, "Rate limit exceeded")
    try:
        ...
        sig = IndicatorSignal(symbol=symbol, timeframe=timeframe,
