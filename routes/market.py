...
@router.get("/top-volume-with-prices", summary="Top symbols with markPrice + fundingRate (from cache)")
async def get_top_volume_with_prices(
    request: Request,
    market: str = Query("futures", enum=["futures", "spot"]),
    quote: str = Query("USDT"),
    limit: int = Query(20, ge=1, le=50),   # ⬅️ מוגבל ל־50
    min_quote_volume: float = Query(0.0, ge=0.0),
) -> Dict[str, Any]:
    ip = request.client.host
    ...
    try:
        ...
        for sym in symbols or []:
            ...
            enriched.append({**sym, "price": info.get("price"), "fundingRate": info.get("fundingRate"),
                             "nextFundingTime": info.get("nextFundingTime"), "ts": info.get("ts")})
