...
# =========================
# Multi-scan: משיכת נתוני Klines מרובים מ-Binance
# חישוב אינדיקטורים טכניים (RSI, EMA, ATR, ADX וכו׳)
# =========================
...
def _fetch_klines(symbol: str, interval: str = "15m", limit: int = 200) -> pd.DataFrame:
    """
    מחזיר DataFrame עם נתוני Klines מה-Binance (Futures).
    """
...
async def scan_info(
    ...
    """
    סריקה בודדת: מחזירה אינדיקטורים על סימבול יחיד.
    """
    try:
...
async def scan_symbols(
    ...
    """
    סריקה מרובה: מקבלת רשימת סימבולים ומחזירה אינדיקטורים לכל אחד.
    """
    out: List[ScanSignal] = []
    for s in symbols:
        try:
