...
def _entry_fill_probability(distance_pct: float, adx: Optional[float], atrp: Optional[float], aligned: Optional[bool]) -> int:
    ...
    if d <= 0.10: base = 88.0
    elif d <= 0.30: base = 78.0 - (d - 0.10) * (20.0 / 0.20)   # 78→58
    elif d <= 0.60: base = 58.0 - (d - 0.30) * (23.0 / 0.30)   # 58→35
    elif d <= 1.00: base = 35.0 - (d - 0.60) * (10.0 / 0.40)   # 35→25
    elif d <= 2.00: base = 25.0 - (d - 1.00) * (10.0 / 1.00)   # 25→15
    else: base = 12.0
    if adx is not None:
        a = float(adx)
        if a >= 35: base += 8.0
...
def _tp_tiers(symbol: str, entry: float, direction: str, atr: Optional[float]) -> List[Dict[str, Any]]:
    ...
    floor = float(params.get("tp_pct_floor", 0.006))   # 0.6% רצפה
...
async def _build_ready_plan(item: Dict[str, Any], market_type: str, budget_usd: float, btc_strength: Optional[float], btc_dir: Optional[str]) -> Dict[str, Any]:
    ...
    if qty_ok:
        ...
        # order_template — כולל סכומי $ לכל TP
        order_tmpl = {
            "symbol": sym,
            "side": direction,          # ב-Futures: LONG=BUY, SHORT=SELL
            "type": "LIMIT",
            "price": entry_str,
            "quantity": qty_str,
            "stop_loss": sl_str,
        ...
        plan.update({
            ...
            "tp_tiers": tiers,  # בלי $PnL כי אין כמות
            "order_template": {
                "symbol": sym,
                "side": direction,
                "type": "LIMIT",
...
async def scan_multi(
    ...
    include_results: Optional[bool] = Query(True, description="האם להחזיר גם את הנתונים הגולמיים (results) בנוסף ל-ready"),
):
    try:
        ...
        if bz < 50.0:
            bz = 50.0  # מינימום קשיח שביקשת

        # בניית ready לכל פריט
