...
from utils.indicators_ext import add_extended_indicators, extended_score_last_row  # קיים אצלך

FUTURES_BASE = os.getenv("BINANCE_FUTURES_HTTP_BASE", "https://fapi.binance.com")

_S = requests.Session()
_S.trust_env = False
_S.headers.update({"User-Agent": "AlgoGPT/2 scan-top-vol+ext", "Accept": "application/json", "Accept-Encoding": "gzip"})
...
def _klines(symbol: str, interval: str, limit: int) -> Optional[pd.DataFrame]:
    try:
        ...
        if r.status_code != 200:
            return None
        arr = r.json()
        df = pd.DataFrame(arr, columns=["openTime","open","high","low","close","volume","closeTime","qv","nTrades","takerBase","takerQuote","x"])
...
async def _scan_one(symbol: str, timeframe: str, bars: int, *, ema_fast: int, ema_slow: int, adx_len: int,
                    st_period: int, st_factor: float, ich_conv: int, ich_base: int, ich_span_b: int,
                    ms_lookback: int, ms_pivot_span: int, min_adx: float, trending_only: bool) -> Optional[Dict[str, Any]]:
    ...
    return {"symbol": symbol, "timeframe": timeframe, "side": side, "score": score, "note": reason, "details": details}
...
async def scan_top_volume(
    ...
    results = await asyncio.gather(*[asyncio.create_task(_wrapped(s)) for s in symbols], return_exceptions=True)
    out: List[Dict[str, Any]] = [r for r in results if isinstance(r, dict)]
