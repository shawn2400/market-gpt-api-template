...
def _rl(ip: str, limit=30, window=60):
    ...
    calls.append(now); _rate_state[ip] = calls; return True
...
class IndicatorSet(BaseModel):
    rsi: Optional[float] = None; ema_21: Optional[float] = None
    adx: Optional[float] = None; atr: Optional[float] = None
    vwap_trend: Optional[bool] = None

class ScanSignal(BaseModel):
    symbol: str; interval: str; indicators: Optional[IndicatorSet] = None
    ok: bool = True; error: Optional[str] = None

class MultiScanResponse(BaseModel):
    ok: bool = True; count_total: int; returned: int
    signals: List[ScanSignal] = Field(default_factory=list); error: Optional[str] = None

@router.get("/info", response_model=MultiScanResponse)
async def scan_info(symbol: str = Query(...), interval: str = Query("15m"),
                    limit: int = Query(200, ge=50, le=200), request: Request = None) -> MultiScanResponse:
    if not _rl(request.client.host): raise HTTPException(429, "Rate limit exceeded")
    try:
        df = _fetch_klines(symbol, interval, limit)
        if df.empty: return MultiScanResponse(ok=False, count_total=1, returned=0, signals=[], error="no data")
        ind = prepare_indicators_for_backtest(df); row = ind.iloc[-1].to_dict()
...
async def scan_symbols(symbols: List[str] = Query(...), interval: str = Query("15m"),
                       limit: int = Query(200, ge=50, le=200), request: Request = None) -> MultiScanResponse:
    if not _rl(request.client.host): raise HTTPException(429, "Rate limit exceeded")
    out: List[ScanSignal] = []
    for s in symbols:
        try:
            ...
            if df.empty: out.append(ScanSignal(symbol=s, interval=interval, ok=False, error="no data")); continue
            ind = prepare_indicators_for_backtest(df); row = ind.iloc[-1].to_dict()
            out.append(ScanSignal(symbol=s, interval=interval, indicators=IndicatorSet(**row)))
        except Exception as e: out.append(ScanSignal(symbol=s, interval=interval, ok=False, error=str(e)))
