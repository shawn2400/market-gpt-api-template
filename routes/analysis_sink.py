...
try:
    ...
    def require_api_key(): return None
...
def _get_trade(tid: str): return RED.hgetall(f"trades:active:{tid}") if USE_REDIS_TRADES else _TRADES.get(tid)
...
def _update_trade(tid: str, **updates):
    ...
    elif tid in _TRADES: _TRADES[tid].update(updates)
...
def _format_trade_message(rec: Dict[str,Any]) -> str:
    ...
    if rec.get("notional_usd"): lines.append(f"Budget: ${rec.get('budget_usd') or '—'}  Notional: ${float(rec['notional_usd']):.2f}  Qty: {fmt(rec.get('qty'))}")
...
async def trade_ingest(payload: TradeIn=Body(...),
    ...
    rec.update({"status":"active","ts":int(time.time()),"hits":json.dumps({"tp1":False,"tp2":False,"tp3":False,"sl":False}),"near":json.dumps({"tp1":False,"tp2":False,"tp3":False,"sl":False})})
    txt=_format_trade_message(rec)
    body={"chat_id":payload.chat_id or CHAT_ID_DEFAULT,"text":txt,"parse_mode":"Markdown","disable_web_page_preview":True,"reply_markup":_approve_keyboard(payload.trade_id)}
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        ...
        _store_trade(rec); return TradeOut(ok=True, trade_id=payload.trade_id, message_id=rec["message_id"], chat_id=rec["chat_id"])

class ActiveOut(BaseModel): ok: bool=True; count:int; items:List[Dict[str,Any]]
@router.get("/trades/active",response_model=ActiveOut)
def trades_active(): items=_all_active(); return ActiveOut(ok=True,count=len(items),items=items)

class UpdateReq(BaseModel): trade_id:str; updates:Dict[str,Any]
@router.post("/trades/update",response_model=dict)
def trades_update(payload:UpdateReq): _update_trade(payload.trade_id,**payload.updates); return {"ok":True}

class AnalysisIn(BaseModel): chat_id:int|str; text:str; reply_to_message_id:Optional[int]=None; silent:Optional[bool]=True
...
async def analysis_ingest(payload:AnalysisIn=Body(...),
    ...
    if not BOT: raise HTTPException(500,"TELEGRAM_BOT_TOKEN not configured")
    body={"chat_id":payload.chat_id,"text":payload.text,"parse_mode":"Markdown","disable_web_page_preview":True}
