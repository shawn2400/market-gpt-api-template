...
def _store_trade(item: Dict[str, Any]):
    if USE_REDIS_TRADES:
        RED.hset(f"trades:active:{item['trade_id']}", mapping=item)  # type: ignore
        RED.sadd("trades:active:set", item["trade_id"])               # type: ignore
    else:
...
def _all_active() -> List[Dict[str, Any]]:
    if USE_REDIS_TRADES:
        ...
        for tid in RED.smembers("trades:active:set"):  # type: ignore
            rec = RED.hgetall(f"trades:active:{tid}")  # type: ignore
...
def _format_trade_message(rec: Dict[str, Any]) -> str:
    lines = [
        ...
        f"Now: `{_fmt_num(rec.get('current_price'))}`  Entry: `{_fmt_num(rec.get('entry'))}`",
        f"SL: `{_fmt_num(rec.get('sl'))}`  TP1: `{_fmt_num(rec.get('tp1'))}`  TP2: `{_fmt_num(rec.get('tp2'))}`  TP3: `{_fmt_num(rec.get('tp3'))}`",
    ]
    if rec.get("success_pct") is not None:
        try:
            lines.append(f"Success: *{float(rec['success_pct'])*100:.1f}%*")
        except Exception:
            pass
    if rec.get("notional_usd") is not None or rec.get("budget_usd") is not None or rec.get("qty") is not None:
        lines.append(
            f"Budget: ${rec.get('budget_usd') or '—'}  Notional: ${_fmt_num(rec.get('notional_usd'))}  Qty: {_fmt_num(rec.get('qty'))}"
        )
    if rec.get("reason"):
...
async def trade_ingest(
    ...
    if not BOT:  # מגן קשה – לא מרימים בקשות לטלגרם בלי טוקן
