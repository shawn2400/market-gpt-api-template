...
def _collect_symbol(cli, symbol: str) -> Dict[str, Any]:
    # תקין: הפונקציה סינכרונית (הריצה ב-executor הייתה בעייתית כשהייתה async)
    symbol = symbol.upper()
...
async def publish_positions_once() -> Dict[str, Any]:
    ...
    for sym in _WATCHLIST:
        try:
            # תקין: איסוף בסנכרון דרך executor
            doc = await loop.run_in_executor(None, _collect_symbol, cli, sym)
            out.append(doc)
            # key פר-סימבול
