...
import os, time, logging, asyncio, math
...
from utils.binance_client import (
    ...
    get_all_orders, futures_cancel_order, get_price_coalesced,
)
...
def _order_matches(
    o: Dict[str, Any], *,
    typ: str, qty: str, stop: Optional[str], price: Optional[str],
...
def _place_close_order_hardened(
    args: Dict[str, Any], *, sym: str, typ: str, qty_str: str,
    stop_str: Optional[str], price_str: Optional[str],
    side: str, eff_ps: str, expect_ro: bool,
    place_fn, list_fn, cancel_fn, max_retries: int = 3
) -> Dict[str, Any]:
    """
    יוצר פקודת TP/SL (reduceOnly) עם אימות ע"י רשימת ההזמנות, וביטול־וניסיון מחדש במקרה של אי־התאמה.
    """
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if cand and _order_matches(
                cand, typ=typ, qty=qty_str, stop=stop_str, price=price_str,
...
# ─────────── Utilities: כיווץ נתונים, ניקוי, ועוד ───────────
...
def _ensure_qty(sym: str, qty: Optional[float], *, budget_usdt: Optional[float],
                price: Optional[float], leverage: Optional[float]) -> Tuple[str, float]:
    """
    קובע כמות מזוקקת (לצעד) ומחזיר (qty_str, qty_float). אם לא נמסרה כמות — ניסוי ע"פ תקציב/מחיר.
    לא מעלה כמות מעל המינימלי לפר ההגדרות, אך יכבד MIN_NOTIONAL.
    """
...
def _approve_if_needed(idem: _Idem, plan: Dict[str, Any]) -> None:
    """
    שולח בקשת אישור אנושית אם נדרש (מדיניות/ENV/דגל ב־plan). נחסם עד תוצאת אישור.
    """
...
def _place_tp_sl_for_position(
    ...
    *,
    ...
    """
    תוחם TP/SL בסדרה אחת. מוודא reduceOnly ומאמת הזמנה.
    """
    ...
    for i, leg in enumerate(tp_targets, start=1):
        ...
        args = {
            ...
            "stopPrice": _q_price(symbol, px),  # BINANCE דורש price + stopPrice ב-TP LIMIT לפעמים
            "timeInForce": "GTC",
        ...
        res = _place_close_order_hardened(
            args, sym=symbol, typ="TAKE_PROFIT", qty_str=part_qty_str,
            stop_str=args["stopPrice"], price_str=args["price"],
            side=close_side, eff_ps=eff_position_side, expect_ro=True,
    ...
    if sl_target and float(sl_target.get("price") or 0.0) > 0:
        ...
        sl_res = _place_close_order_hardened(
            sl_args, sym=symbol, typ="STOP", qty_str=qty_str,
            stop_str=sl_args["stopPrice"], price_str=sl_args["price"],
            side=close_side, eff_ps=eff_position_side, expect_ro=True,
            place_fn=futures_create_order, list_fn=get_all_orders, cancel_fn=futures_cancel_order,
        )
        results["sl"] = sl_res
...
def _place_hybrid_entry(
    ...
    *,
    ...
    """
    HYBRID: מציב LIMIT באזור המחיר (band_bps) ומסלים ל-MARKET אחרי פרק זמן, עם סטייה מותרת נוספת.
    """
    side_u = side.upper()
    # מחיר יעד ללימיט בתוך band
    ...
    # הסלמה ל־MARKET עם סטייה נוספס
    # guard: אל תאפשר MARKET אם PERCENT_PRICE_GUARD נשבר
    ref_px = float(_maybe_price(symbol) or entry_ref_px)
    mkt_px = _offset_bps(ref_px, +escalate_slip_bps if side_u == "BUY" else -escalate_slip_bps)
    mkt_args = {
        "symbol": symbol.upper(),
        "side": side_u,
        "type": "MARKET",
    ...
    # אין מחיר ב־MARKET, אבל נעשה sanity: אם השוק ברח חזק מדי — ניתן להשאיר למעלה את האחריות לשכבה חיצונית.
...
def _place_direct_entry(
    ...
    *,
    ...
    """
    MARKET / LIMIT ישיר לפי order_type. ב-LIMIT חובה price.
    """
    side_u = side.upper()
...
def _compute_dynamic_sl_price(
    side: str, ref_price: float, atr_val: Optional[float], *, atr_mult: float
) -> Optional[float]:
...
def _maybe_breakeven_after_tp1(
    ...
    """
    אופציונלי: לאחר TP1, מקדם SL ל-BE (+offset). מנגנון זה בד"כ דורש מעקב מתמשך/worker.
    כאן אנו מציבים SL חדש ב־reduceOnly במקום הקודם (פשוט: ביטול הישן והצבת חדש).
    אם אין מנוע חיצוני, הפונקציה תבצע חישוב חד-פעמי בלבד (Best-effort).
    """
    if not BE_GUARD_ENABLE:
        return None
    # תנאי מפשט: אם התוויות כוללות TP1 או שהוגדר בסביבה.
    ...
    if eff_position_side != "BOTH":
        args["positionSide"] = eff_position_side
    res = _place_close_order_hardened(
        args, sym=symbol, typ="STOP", qty_str=qty_str,
        stop_str=args["stopPrice"], price_str=args["price"],
        side=close_side, eff_ps=eff_position_side, expect_ro=True,
        place_fn=futures_create_order, list_fn=get_all_orders, cancel_fn=futures_cancel_order,
    )
...
def execute_trade_live(plan: Dict[str, Any]) -> Dict[str, Any]:
    """
    מבצע טרייד ע"פ plan וה־ENV. כולל:
      • אימות איכות/ATR/נפח (באמצעות _quality_gate)
      • אישור אנושי (אופציונלי)
      • הצבת הזמנת כניסה (MARKET/LIMIT/HYBRID)
      • Ladder TP/SL (reduceOnly) + מגיני BE/Trail (Best effort)
    מחזיר dict עם "ok" ופרטי הזמנות.
    """
    t0 = time.time()
    ...
    # יישור מצב פוזיציות (Hedge/OneWay) אם הוגדר ENV
    ...
    atr = None
    with suppress(Exception):
        adx = _adx_from_klines(kl) if kl else None
    with suppress(Exception):
        atr = _atr_from_klines(kl) if kl else None

    quality = float(plan.get("score") or plan.get("quality") or QUALITY_DEFAULT)
    # gate (throws on reject) — נחזיר שגיאה במקום חריגה
    ok, reason_bad = _quality_gate(
        quality=quality, min_score=MIN_QUALITY_SCORE or MIN_QUALITY_FALLBACK,
        atr_pct=plan.get("atr_pct"), atr_abs=atr, max_atr_pct=MAX_ATR_PCT,
        volume=plan.get("vol"), min_volume=MIN_VOLUME,
    )
    if not ok:
    ...
    # מינוף דינמי
    leverage = _choose_leverage(plan, adx=adx, lev_map_json=LEV_ADX_MAP_JSON)
    ...
    if not entry_ref_px:
        # נסה לסחוט מחיר גם אם נעדר
        p = _maybe_price(symbol)
        entry_ref_px = float(p or 0.0)
    if entry_ref_px <= 0:
        return {"ok": False, "error": "no_price_available"}

    dyn_budget = _choose_budget_dynamic(
        plan,
        enable=BUDGET_DYNAMIC_ENABLE,
        use_balance=BUDGET_USE_BALANCE,
        risk_pcts=BUDGET_DYNAMIC_RISK_PCTS,
        price=entry_ref_px,
        quality=quality,
        atr=atr,
    )
    # Fallback: פונקציית חוץ אם אין דינמי
    ...
    # צד פוזיציה אפקטיבי (BOTH/LONG/SHORT)
    eff_ps = _effective_position_side(side_in, hedge_runtime=_is_hedge_mode_runtime())
    ...
    if order_type == "HYBRID":
        ...
        entry_res = _place_direct_entry(
            symbol, side_in, qty_str,
            entry_price=float(plan.get("limit_price") or plan.get("entry_price") or entry_ref_px) if order_type=="LIMIT" else None,
            order_type=order_type,
            eff_position_side=eff_ps,
            client_tag="DIR",
        )

    # חשב TP/SL (לפי תצורת Ladder ו/או יעדים שניתנו ב-plan)
    # _compute_tp_sl_targets מחזיר {"tp":[{price,qty},...], "sl":{price,qty?}}
    ...
    # ניתן לעדכן dyn-SL אם atr קיים ולא סופק SL מפורש
    ...
    # מגיני BE אופציונלי (best-effort, חד־פעמי)
    be_res = None
    with suppress(Exception):
        be_res = _maybe_breakeven_after_tp1(
            plan=plan, symbol=symbol, side=side_in,
            entry_price=entry_ref_px, eff_position_side=eff_ps, qty_str=qty_str
        )

    # (אופציונלי) טריילינג סטופ — בפועל נדרש וורקר/סדולר; כאן רק חישוב והצבה חד-פעמית אם ביקשו
...
def _safe_close_position(symbol: str, side_opened: str, qty: float, *, eff_position_side: str) -> Dict[str, Any]:
    """
    סגירה מיידית של פוזיציה: יצירת MARKET הפוך ב-reduceOnly.
    """
...
__all__ = [
    "execute_trade_live", "execute_trade_live_async",
    "_safe_close_position",
]
