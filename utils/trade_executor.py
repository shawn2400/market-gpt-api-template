...
import httpx
from utils.binance_client import get_price, futures_mark_price, set_leverage, futures_create_order, get_all_orders, futures_cancel_order, get_price_coalesced
from utils.trade_execution_core import ALLOW_MARKET_ENTRY, ENTRY_BAND_BPS, STOP_BAND_BPS, ESCALATE_AFTER_S, ESCALATE_SLIP_BPS, PERCENT_PRICE_GUARD_BPS, SLIPPAGE_GUARD_BPS, POST_FILL_SANITY_BPS, ENFORCE_POST_FILL_SANITY, QUALITY_DEFAULT, MIN_QUALITY_SCORE, MIN_QUALITY_FALLBACK, MAX_ATR_PCT, MIN_VOLUME, ENFORCE_APPROVAL_ALWAYS, REQUIRE_TP_AND_SL, LADDER_TP_ENABLE, LADDER_TP_KIND, LADDER_TP_DEFAULT_PCTS, LADDER_TP_DEFAULT_SPLITS, LADDER_SL_ENABLE, LADDER_SL_DEFAULT_PCTS, TRAIL_CALLBACK_MIN_PCT, TRAIL_CALLBACK_MAX_PCT, BUDGET_DYNAMIC_ENABLE, BUDGET_USE_BALANCE, BUDGET_DYNAMIC_RISK_PCTS, DYN_LEVERAGE_ENABLE, MIN_LEVERAGE, LEV_HARD_CAP, LEV_ADX_MAP_JSON, ORDER_ID_PREFIX, CANCEL_ONLY_PREFIXED_ORDERS, CANCEL_PREFIX_OVERRIDE, IDEMPOTENCY_TTL_SEC, BOT_TOKEN, API_BASE, CONFIRM_TTL_SEC, TELEGRAM_CHAT_ID, TELEGRAM_PARSE_MODE, _q_price, _q_qty, _ensure_min_notional, _calc_qty, _offset_bps, _is_hedge_mode_runtime, _effective_position_side, _fetch_klines_raw, _adx_from_klines, _atr_from_klines, _quality_gate, _choose_budget_dynamic, _choose_leverage, _parse_csv_floats, _cancel_old_closing_orders, _normalize_position_side, _close_side_for, _pos_side_for_entry, _normalize_entry_side, _compute_tp_sl_targets, _compute_trailing_callback_pct, _Idem
...
try:
    ...
    def get_budget_usdt(symbol: Optional[str] = None, *, quality: Optional[float] = None, atr: Optional[float] = None, price: Optional[float] = None) -> float:
        try:
...
def _order_matches(o: Dict[str, Any], *, typ: str, qty: str, stop: Optional[str], price: Optional[str], side: str, eff_ps: str, expect_ro: bool) -> bool:
    try:
        ...
        if eff_ps == "BOTH" and ("positionSide" in o): return False
        if expect_ro:
            ro = o.get("reduceOnly")
...
def _place_close_order_hardened(args: Dict[str, Any], *, sym: str, typ: str, qty_str: str, stop_str: Optional[str], price_str: Optional[str], side: str, eff_ps: str, expect_ro: bool, place_fn, list_fn, cancel_fn, max_retries: int = 3) -> Dict[str, Any]:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if cand and _order_matches(cand, typ=typ, qty=qty_str, stop=stop_str, price=price_str, side=side, eff_ps=eff_ps, expect_ro=expect_ro):
                ok = True
        except Exception as e:
...
def _ensure_qty(sym: str, qty: Optional[float], *, budget_usdt: Optional[float], price: Optional[float], leverage: Optional[float]) -> Tuple[str, float]:
...
def _place_tp_sl_for_position(symbol: str, side_entry: str, qty_str: str, entry_price: Optional[float], tp_targets: List[Dict[str, Any]], sl_target: Optional[Dict[str, Any]], *, eff_position_side: str, client_tag: str) -> Dict[str, Any]:
    ...
    for i, leg in enumerate(tp_targets, start=1):
        ...
        args = {"symbol": symbol.upper(), "side": close_side, "type": "TAKE_PROFIT", "reduceOnly": True, "quantity": part_qty_str, "price": _q_price(symbol, px), "stopPrice": _q_price(symbol, px), "timeInForce": "GTC", "newClientOrderId": _coid(f"TP{i}", symbol, close_side) if ORDER_ID_PREFIX else None}
        if eff_position_side != "BOTH":
            args["positionSide"] = eff_position_side
        res = _place_close_order_hardened(args, sym=symbol, typ="TAKE_PROFIT", qty_str=part_qty_str, stop_str=args["stopPrice"], price_str=args["price"], side=close_side, eff_ps=eff_position_side, expect_ro=True, place_fn=futures_create_order, list_fn=get_all_orders, cancel_fn=futures_cancel_order)
        results["tp"].append(res)
    if sl_target and float(sl_target.get("price") or 0.0) > 0:
        sl_px = float(sl_target["price"])
        sl_args = {"symbol": symbol.upper(), "side": close_side, "type": "STOP", "reduceOnly": True, "quantity": qty_str, "price": _q_price(symbol, sl_px), "stopPrice": _q_price(symbol, sl_px), "timeInForce": "GTC", "newClientOrderId": _coid("SL", symbol, close_side) if ORDER_ID_PREFIX else None}
        if eff_position_side != "BOTH":
            sl_args["positionSide"] = eff_position_side
        sl_res = _place_close_order_hardened(sl_args, sym=symbol, typ="STOP", qty_str=qty_str, stop_str=sl_args["stopPrice"], price_str=sl_args["price"], side=close_side, eff_ps=eff_position_side, expect_ro=True, place_fn=futures_create_order, list_fn=get_all_orders, cancel_fn=futures_cancel_order)
        results["sl"] = sl_res
    return results

def _place_hybrid_entry(symbol: str, side: str, qty_str: str, *, entry_ref_px: float, band_bps: float, escalate_after_s: int, escalate_slip_bps: float, eff_position_side: str, client_tag: str) -> Dict[str, Any]:
    side_u = side.upper()
    limit_px = _offset_bps(entry_ref_px, -band_bps if side_u == "BUY" else band_bps)
    limit_args = {"symbol": symbol.upper(), "side": side_u, "type": "LIMIT", "quantity": qty_str, "price": _q_price(symbol, float(limit_px)), "timeInForce": "GTC", "newClientOrderId": _coid("HYB_LMT", symbol, side_u) if ORDER_ID_PREFIX else None}
    if eff_position_side != "BOTH":
    ...
    if oid is not None and HYBRID_HARD_CANCEL_ENABLE:
        try:
            ...
            for _ in range(HYBRID_CANCEL_CONFIRM_TRIES):
                ...
                still = next((o for o in oo if str(o.get("orderId")) == str(oid) and (o.get("status","")).upper()=="NEW"), None)
                if not still:
                    break
        except Exception as e:
    ...
    mkt_args = {"symbol": symbol.upper(), "side": side_u, "type": "MARKET", "quantity": qty_str, "newClientOrderId": _coid("HYB_MKT", symbol, side_u) if ORDER_ID_PREFIX else None}
    if eff_position_side != "BOTH":
        mkt_args["positionSide"] = eff_position_side
    out["entry_market"] = futures_create_order(**mkt_args)
    return out

def _place_direct_entry(symbol: str, side: str, qty_str: str, entry_price: Optional[float], *, order_type: str, eff_position_side: str, client_tag: str) -> Dict[str, Any]:
    side_u = side.upper()
    typ = order_type.upper()
    if typ == "MARKET":
        args = {"symbol": symbol.upper(), "side": side_u, "type": "MARKET", "quantity": qty_str, "newClientOrderId": _coid("MKT", symbol, side_u) if ORDER_ID_PREFIX else None}
        if eff_position_side != "BOTH":
            args["positionSide"] = eff_position_side
        ...
        args = {"symbol": symbol.upper(), "side": side_u, "type": "LIMIT", "timeInForce": "GTC", "quantity": qty_str, "price": _q_price(symbol, float(entry_price)), "newClientOrderId": _coid("LMT", symbol, side_u) if ORDER_ID_PREFIX else None}
        if eff_position_side != "BOTH":
            args["positionSide"] = eff_position_side
        res = futures_create_order(**args)
...
def _maybe_breakeven_after_tp1(plan: Dict[str, Any], symbol: str, side: str, entry_price: float, eff_position_side: str, qty_str: str) -> Optional[Dict[str, Any]]:
    ...
    args = {"symbol": symbol.upper(), "side": close_side, "type": "STOP", "reduceOnly": True, "quantity": qty_str, "price": _q_price(symbol, float(be_px)), "stopPrice": _q_price(symbol, float(be_px)), "timeInForce": "GTC", "newClientOrderId": _coid("SL_BE", symbol, close_side) if ORDER_ID_PREFIX else None}
    if eff_position_side != "BOTH":
        args["positionSide"] = eff_position_side
    res = _place_close_order_hardened(args, sym=symbol, typ="STOP", qty_str=qty_str, stop_str=args["stopPrice"], price_str=args["price"], side=close_side, eff_ps=eff_position_side, expect_ro=True, place_fn=futures_create_order, list_fn=get_all_orders, cancel_fn=futures_cancel_order)
    return res
...
def execute_trade_live(plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ok, reason_bad = _quality_gate(quality=quality, min_score=MIN_QUALITY_SCORE or MIN_QUALITY_FALLBACK, atr_pct=plan.get("atr_pct"), atr_abs=atr, max_atr_pct=MAX_ATR_PCT, volume=plan.get("vol"), min_volume=MIN_VOLUME)
    if not ok:
    ...
    leverage = _choose_leverage(symbol, float(adx), int(plan.get("leverage") or plan.get("lev") or MIN_LEVERAGE))
    ...
    qty_str, qty_f = _ensure_qty(symbol, qty=plan.get("qty"), budget_usdt=dyn_budget, price=entry_ref_px, leverage=leverage)
    eff_ps = _effective_position_side(plan.get("position_side") or _pos_side_for_entry(side_in), hedge_runtime=_is_hedge_mode_runtime())
    order_type = str(plan.get("order_type") or plan.get("entry_type") or "MARKET").upper()
    if order_type == "HYBRID":
        entry_res = _place_hybrid_entry(symbol, side_in, qty_str, entry_ref_px=entry_ref_px, band_bps=float(ENTRY_BAND_BPS or 0.0), escalate_after_s=int(ESCALATE_AFTER_S or 6), escalate_slip_bps=float(ESCALATE_SLIP_BPS or 0.0), eff_position_side=eff_ps, client_tag="HYB")
    else:
        entry_res = _place_direct_entry(symbol, side_in, qty_str, entry_price=float(plan.get("limit_price") or plan.get("entry_price") or entry_ref_px) if order_type=="LIMIT" else None, order_type=order_type, eff_position_side=eff_ps, client_tag="DIR")
    targets = _compute_tp_sl_targets(symbol=symbol, side=side_in, qty=qty_f, price_ref=entry_ref_px, plan=plan, ladder_tp_enable=LADDER_TP_ENABLE, ladder_tp_kind=LADDER_TP_KIND, ladder_tp_default_pcts=LADDER_TP_DEFAULT_PCTS, ladder_tp_default_splits=LADDER_TP_DEFAULT_SPLITS, ladder_sl_enable=LADDER_SL_ENABLE, ladder_sl_default_pcts=LADDER_SL_DEFAULT_PCTS, sl_dynamic_enable=SL_DYNAMIC_ENABLE, sl_atr_mult=SL_ATR_MULT, atr_abs=atr, stop_band_bps=STOP_BAND_BPS)
    ...
    tpsl_res = _place_tp_sl_for_position(symbol=symbol, side_entry=side_in, qty_str=qty_str, entry_price=entry_ref_px, tp_targets=targets.get("tp") or [], sl_target=targets.get("sl"), eff_position_side=eff_ps, client_tag="CLS")
    be_res = None
    with suppress(Exception):
        be_res = _maybe_breakeven_after_tp1(plan=plan, symbol=symbol, side=side_in, entry_price=entry_ref_px, eff_position_side=eff_ps, qty_str=qty_str)
    trail_enable = bool(plan.get("trail_enable", TRAIL_ENABLE_DEFAULT))
    trail_cb_pct = None
    if trail_enable:
        try:
            trail_cb_pct = _compute_trailing_callback_pct(plan=plan, atr_abs=atr, min_pct=TRAIL_CALLBACK_MIN_PCT, max_pct=TRAIL_CALLBACK_MAX_PCT, default_mult=TRAIL_ATR_MULT_DEFAULT)
        except Exception as e:
    ...
    return {"ok": True, "symbol": symbol, "side": side_in, "leverage": leverage, "qty": qty_str, "entry": entry_res, "tpsl": tpsl_res, "be": be_res, "trail": {"enabled": trail_enable, "callback_pct": trail_cb_pct, "note": "requires external worker for dynamic updates" if trail_enable else None}, "elapsed_sec": round(elapsed, 3)}
...
def _safe_close_position(symbol: str, side_opened: str, qty: float, *, eff_position_side: str) -> Dict[str, Any]:
    ...
    args = {"symbol": symbol.upper(), "side": close_side, "type": "MARKET", "reduceOnly": True, "quantity": qty_str, "newClientOrderId": _coid("CLOSE", symbol, close_side) if ORDER_ID_PREFIX else None}
    if eff_position_side != "BOTH":
        args["positionSide"] = eff_position_side
    try:
        res = futures_create_order(**args)
