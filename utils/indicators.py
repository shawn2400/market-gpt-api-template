...
def _supertrend(df: pd.DataFrame, period: int = _ST_PERIOD, multiplier: float = _ST_MULT) -> pd.Series:
    atr = AverageTrueRange(
        high=df["high"], low=df["low"], close=df["close"], window=period, fillna=True
    ).average_true_range()
...
def compute_indicators(df: Optional[pd.DataFrame]) -> pd.DataFrame:
    """
    מחזיר DataFrame עם:
    rsi, adx, atr, macd, macd_signal, macd_hist, ema_21, ema_50,
    vwap, supertrend, supertrend_dir, volume_mean, pattern, דגלי תבניות, ו-trend.
    """
    base = _ensure_df(df)
    if base.empty:
        return pd.DataFrame()
    ...
    out = base.copy()
    try:
        # EMA (ללא walrus – תקין בכל פייתון 3.8+)
