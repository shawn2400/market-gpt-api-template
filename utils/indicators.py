...
# ------------------------------------------------------------
# Core indicator preparation
# ------------------------------------------------------------

def prepare_indicators_for_backtest(df: pd.DataFrame) -> pd.DataFrame:
    """
    מכין DataFrame עם עמודות אינדיקטורים בסיסיים לשימוש בבק-טסט/סריקה.
    לא מפיל על NaN — ממיר מספרים, מנקה, ומחזיר DF תקין ככל האפשר.
    דרישות עמודות קלט: ['open','high','low','close','volume'] + אופציונלי 'open_time' / 'timestamp'
    """
    ...
    # הורדת שורות שאי אפשר לחשב עליהן אינדיקטורים
    ...
    # חותמת זמן: normalize to 'timestamp'
    if "timestamp" not in work.columns:
        if "open_time" in work.columns:
            work.rename(columns={"open_time": "timestamp"}, inplace=True)
        elif "openTime" in work.columns:
            work.rename(columns={"openTime": "timestamp"}, inplace=True)
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Alias היסטורי — שומר תאימות לקוד שקורא compute_indicators().
    """
    return prepare_indicators_for_backtest(df)

def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Alias היסטורי — שומר תאימות לקוד שקורא add_indicators().
    """
    return prepare_indicators_for_backtest(df)
