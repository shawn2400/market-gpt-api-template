...
# --- פונקציית חישוב אינדיקטורים מלאה עם טיפול בנתונים חסרים ---
def compute_indicators(df, volume_window=20):
    if df.empty or any(col not in df.columns for col in ['open','high','low','close','volume']):
        logging.warning("[!] df לא תקין לחישוב אינדיקטורים - חסרים עמודות חיוניות או ריק")
        return df
...
# --- פונקציות עזר ל־Backtest ול־Live Scan ---
