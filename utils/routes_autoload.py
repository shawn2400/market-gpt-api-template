...
from typing import Iterable, Optional, Set, Iterator
...
def _call_setup_if_present(app: FastAPI, module: ModuleType, verbose: bool = False) -> bool:
    ...
    if callable(setup):
        try:
            ...
            # נרשום אזהרה אך נמשיך — ייתכן שעדיין יש router גלובלי
            if verbose:
                log.warning("routes_autoload: setup(app) failed on %s: %s", module.__name__, e, exc_info=True)
            else:
                log.warning("routes_autoload: setup(app) failed on %s: %s", module.__name__, e)
    return False


def _iter_route_modules(package: str = "routes") -> Iterator[str]:
    """
    סורק מודולים ישירים תחת החבילה `package` ומחזיר את שמותיהם המלאים (fqmn).
    (התנהגות שמרנית — ללא רקורסיה; תואם לקוד המקורי)
    """
    ...
    *,
    ...
    for fqmn in _iter_route_modules(package):
        ...
        try:
            ...
            if verbose:
                log.warning("routes_autoload: import failed for %s: %s", fqmn, e, exc_info=True)
            else:
                log.warning("routes_autoload: import failed for %s: %s", fqmn, e)
            continue

        # IMPORTANT: קודם setup(app), ורק אם לא הותקן כלום — ננסה router גלובלי.
        did_any = _call_setup_if_present(app, mod, verbose=verbose)
        if not did_any:
            did_any = _include_router_if_present(app, mod, verbose=verbose)

        if not did_any and verbose:
