...
def compute_quality_score(df, verbose=False) -> float:
    ...
    try:
        ...
        if not hasattr(last, "get"):
            raise TypeError("שורת הנתונים לא כוללת get")
        ...
        atr = last.get("atr", 0)
        if atr and atr > 0: score += 1; reasons.append("ATR תקין")
        macd = last.get("macd", 0); macd_signal = last.get("macd_signal", 0)
        if macd > macd_signal: score += 1; reasons.append("MACD חיובי")
        rsi = last.get("rsi", 0)
        if 45 <= rsi <= 65: score += 1; reasons.append("RSI נייטרלי")
        adx = last.get("adx", 0)
        if adx > 20: score += 1; reasons.append("ADX חזק")
        vol = last.get("volume", 0); vol_mean = last.get("volume_mean", 1)
        if isinstance(vol, (int, float)) and isinstance(vol_mean, (int, float)) and vol > vol_mean * 1.5:
            score += 1; reasons.append("נפח גבוה")
        close = last.get("close", 0); ema21 = last.get("ema_21", 0)
        ...
        if last.get("macd_hist", 0) > 0: score += 1; reasons.append("MACD היסטוגרמה חיובי")
