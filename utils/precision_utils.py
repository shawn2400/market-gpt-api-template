# File: utils/precision_utils.py
import math
from utils.binance_client import client

def get_precision_info(symbol: str) -> dict:
    """
    מחזיר מידע על דיוק כמות ומחיר עבור סימבול נתון ב-Binance.
    """
    info = client.get_symbol_info(symbol=symbol)

    # דיוק כמות (LOT_SIZE.stepSize)
    lot_filter = next(f for f in info['filters'] if f['filterType'] == 'LOT_SIZE')
    step_size = float(lot_filter['stepSize'])
    quantity_precision = int(round(-math.log10(step_size))) if step_size < 1 else 0

    # דיוק מחיר (PRICE_FILTER.tickSize)
 ...
 def round_to_precision(value: float, precision: int) -> float:
    """
    מעגל ערך נתון לפי מספר הספרות העשרוניות.
    """
