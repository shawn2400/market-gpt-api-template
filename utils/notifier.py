...
class NotifierConfig:
    ...
    always_send_kinds: Tuple[str, ...] = ("eod","digest")  # ← ברירת מחדל בטוחה; המדיניות יכולה לדרוס

def _html(s: str) -> str:
    return s.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")

def _now_ts(fmt_il: str, fmt_utc: str) -> Tuple[str, str]:
...
class Notifier:
    ...
    def __init__(self, policy, session, send_telegram_func):
        ...
        self.cfg = NotifierConfig(
            ...
            always_send_kinds = tuple(n.get("always_send_kinds", ["eod","digest"])),
        )
    ...
    async def send(self, kind: str, text: str, urgent: bool = False, keyboard: Optional[Dict[str, Any]] = None) -> bool:
        ...
        # Always-send or urgent -> bypass buffer & cap (except immediate still counted below if configured)
        ...
        async with self._lock:
            # respect max buffer size; if full, drop oldest and append new (lossy but bounded)
            if len(self.buffer) >=  max(1, 50):
                self.buffer.pop(0)
            self.buffer.append((kind, text))
        return False

    async def _send(self, kind: str, text: str, keyboard: Optional[Dict[str, Any]]):
        # Items in always_send_kinds do NOT count towards cap
        countable = (kind not in self.cfg.always_send_kinds) and (kind not in self.cfg.immediate_kinds)
    ...
    async def flush_digest(self):
        ...
        ts_il, ts_utc = _now_ts("%d/%m/%Y %H:%M:%S %Z", "%Y-%m-%d %H:%M:%S UTC")
        ...
        # שולחים כ-kind="digest" — לא נספר ב-cap לפי always_send_kinds
