...
def compute_quality(
    *,
    ...
    """
    מחזיר: quality_score (0–10) ו־success_pct (%), יחד עם רכיבים מסבירים.
    success_pct כאן היוריסטי עד שתחבר היסטוריית ביצועים אמיתית.
    """
    ...
    leverage_penalty_100 = 30.0 * lev_norm  # עד -30
    ...
    if atr and atr > 0:
        ...
        atr_mult = _safe_div(sl_dist, atr)  # כמה ATR ל-SL
        if atr_mult <= 0:
            atr_score_100 = 10.0
        else:
            # sweet spot סביב 1.5 ATR
            atr_score_100 = 100.0 - _clamp(abs(atr_mult - 1.5) / 1.5 * 40.0, 0.0, 40.0)
    atr_score_100 = _clamp(atr_score_100, 0.0, 100.0)

    # 4) Anchor adjustment (רק רך; HARD נחסם upstream)
    anchor_adj_100 = 0.0
    ...
    # 7) מיפוי סופי
    quality_score = round(combined_100 / 10.0, 2)  # 0–10
    anchor_dir = 0.0
    if anchor and getattr(anchor, "bias", None):
