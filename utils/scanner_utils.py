...
def get_symbols(market_type="futures", min_volume=1_000_000, trending_only=False):
    try:
        ...
        syms = [x["symbol"] for x in info["symbols"] if x["quoteAsset"]=="USDT" and x["status"]=="TRADING"]
        if trending_only:
            tr = set(get_trending_symbols())
...
async def analyze_symbol(symbol, market_type="futures", interval="1m", limit=50, min_volume=1_000_000, trending_only=False, with_ai=True, frames=None):
    async with semaphore:
        ...
        if df.empty:
            return None
        last = df.iloc[-1]
        dirc = ("LONG" if last["rsi"]<35 and last["adx"]>20 and last["close"]>last["ema_21"]
               else "SHORT" if last["rsi"]>70 and last["adx"]>20 and last["close"]<last["ema_21"]
               else "NEUTRAL")
        if dirc=="NEUTRAL":
            return None
        qs = compute_quality_score(df)
        sltp = predict_optimal_sl_tp(symbol, last["close"], dirc) if with_ai else {"sl":None,"tp":None}
        return {
            "symbol":symbol,
...
async def scan_all(market_type="futures", interval="1m", limit=50, min_quality=5, trending_only=False, min_volume=1_000_000, with_ai=True):
    syms = get_symbols(market_type, min_volume, trending_only)[:limit]
    tasks = [analyze_symbol(s, market_type, interval, limit, min_volume, trending_only, with_ai) for s in syms]
