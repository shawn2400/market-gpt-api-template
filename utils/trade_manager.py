...
def _maybe_freeze_trailing(symbol: str, df: pd.DataFrame, atr_now: float, adx_now: float, macd_diff_now: float) -> bool:
    ...
    if spike_mult >= _TRAIL_FREEZE_SPIKE_ATR_MULT:
        ...
        logger.info({"event":"trail_freeze", "symbol":symbol, "dur_sec":dur,
                     "spike_mult":round(spike_mult,2), "adx":round(adx_now,1), "macd_delta":round(macd_diff_now,2)})
        return True
    return False
    ...
    try:
        ...
        for pos in positions:
            try:
                ...
                # Dynamic TP (lock-in under momentum/ADX)
                try:
