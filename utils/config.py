...
OPENAI_TIMEOUT_SECONDS = _env_float("OPENAI_TIMEOUT_SECONDS", 30.0)  # timeout קשיח לקריאה
OPENAI_MAX_CONCURRENCY = _env_int("OPENAI_MAX_CONCURRENCY", 4)      # כמה קריאות GPT במקביל
OPENAI_FALLBACK_MODEL = os.environ.get("OPENAI_FALLBACK_MODEL", "").strip()  # מודל גיבוי (אופציונלי)
...
def log_config_summary():
    logging.info(
        ...
        "SCAN_INTERVAL=%d | MIN_QUALITY_SCORE=%d | MAX_TRADE_BUDGET=%.2f | "
        "MODEL=%s | TIMEOUT=%.1fs | MAX_CONCURRENCY=%d | PORT=%d",
    ...
    if OPENAI_BASE_URL:
        logging.info("[config] OPENAI_BASE_URL=%s", OPENAI_BASE_URL)
    if OPENAI_API_KEY:
