...
BINANCE_SKIP_ACCOUNT_MUTATIONS = os.getenv("BINANCE_SKIP_ACCOUNT_MUTATIONS", "true").lower() in ("1", "true", "yes")
BINANCE_FORCE_HEDGE_MODE = os.getenv("BINANCE_FORCE_HEDGE_MODE", "false").lower() in ("1", "true", "yes")
BINANCE_MARGIN_TYPE_DEFAULT = os.getenv("BINANCE_MARGIN_TYPE_DEFAULT", "ISOLATED").upper()
...
# ---------- Response Limits ----------
RESPONSE_MAX_BYTES = int(os.getenv("RESPONSE_MAX_BYTES", str(2 * 1024 * 1024)))  # default 2 MB
...
def check_config() -> None:
    """Run basic config checks (warnings in dev, fatal in prod)"""
    if not BINANCE_API_KEY or not BINANCE_API_SECRET:
        msg = "❌ Missing Binance API credentials!"
