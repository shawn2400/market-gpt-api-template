...
# קונפיג
_RECV_WINDOW_MS = int(os.getenv("BINANCE_RECV_WINDOW", "45000"))  # 45s ברירת־מחדל בטוחה
_SYNC_TIMEOUT = float(os.getenv("TIME_SYNC_TIMEOUT_SEC", "3.0"))
_SYNC_SKEW_LIMIT_MS = int(os.getenv("TIME_SYNC_SKEW_LIMIT_MS", "2000"))  # אזהרה אם סטייה > 2s
...
def sync_now() -> None:
    """סנכרון חד־פעמי: מעריך offset = server_ms - local_ms"""
    global _offset_ms, _last_sync_mono
    try:
        t0 = _local_time_ms()
        srv = _fetch_server_time_ms()
        t1 = _local_time_ms()
        # אמצע RTT לחישוב הוגן יותר
        mid = (t0 + t1) // 2
...
def start_background_sync(interval_sec: int = 1800) -> None:
    """סנכרון מחזורי (ברירת־מחדל: כל 30 דק’) — דל עומס."""
    def _bg():
        while True:
            try:
...
def ensure_fresh_sync(max_age_sec: int = 3600) -> None:
    """מבטיח שיש סנכרון טרי; אם לא — מבצע sync_now()."""
    with _lock:
...
def last_server_time_ms() -> Optional[int]:
    with _lock:
        if _last_sync_mono <= 0:
            return None
    return server_time_ms()

def recv_window_ms() -> int:
    """חלון recvWindow אחיד לכל קריאה חתומה."""
