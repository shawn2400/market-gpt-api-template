...
# ספים למניעת סטופים/טי־פי צמודים מדי (אופציונלי; ביחידות bps מה־entry)
MIN_ABS_RISK_BPS_DEF   = float(os.getenv("MIN_ABS_RISK_BPS",   "0"))   # 0 = כבוי
MIN_ABS_REWARD_BPS_DEF = float(os.getenv("MIN_ABS_REWARD_BPS", "0"))   # 0 = כבוי
...
def _rr_threshold(vol_regime: str, symbol: str) -> float:
    """
    אם ל־symbol יש min_rr מ־watchlist prefs נשתמש בו; אחרת לפי משטר תנודתיות.
    """
...
def _bps_from_entry(a: float, b: float) -> float:
    """|a-b| ביחידות bps יחסית ל־a (כאן a=entry)."""
    try:
        a = float(a); b = float(b)
        if a <= 0:
            return 0.0
...
def gate_trade(
    ...
    *,
    ...
    # אופציונלי: אילוץ R/R מינימלי חיצוני (יעקוף את vol_regime/prefs)
    ...
    if price is None:
        ...
        try:
            ...
            # אפשר לאפשר התאמה פרטנית מסימבול (אם קיימת)
            prefs = get_symbol_prefs(symbol) or {}
    ...
    # מניעת SL/TP צמודים מדי (bps מוחלטים)
    prefs = get_symbol_prefs(symbol) or {}
