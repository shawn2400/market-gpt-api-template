...
async def _fetch_klines_df(symbol: str):
    r = await safe_get(f"{BINANCE_FAPI}/fapi/v1/klines", params={"symbol":symbol,"interval":DEFAULT_INTERVAL,"limit":ATR_LIMIT})
    arr = r.json()
    import pandas as pd
    ...
    try:
        ...
        for p in pos:
            if str(p.get("symbol","")).upper() == symbol.upper():
                return float(p.get("entry") or 0.0), float(p.get("qty") or 0.0), str(p.get("side") or "")
    except Exception:
        pass
    return 0.0, 0.0, ""
...
def _stage_from_label(label: str) -> Optional[int]:
    L = label.upper()
    if "TP1" in L: return 1
    if "TP2" in L: return 2
    if "TP3" in L: return 3
    return None
...
async def _on_tp(symbol: str, side: str, filled_price: float, label: Optional[str]):
    ...
    if stage is None:
        # ATR-based
        try:
            df = await _fetch_klines_df(symbol); atr_v = _atr(df)
        except Exception:
            atr_v = 0.0
        # קירוב: בחירת stage לפי מרחק מהיעדים
...
async def _futures_listen_key() -> str:
    headers = {"X-MBX-APIKEY": os.getenv("BINANCE_API_KEY","").strip(), "Accept": "application/json"}
...
def _is_reduce_only_tp(o: Dict[str, Any]) -> bool:
    ...
    return ty.startswith("TAKE_PROFIT") and ro and st in ("FILLED","PARTIALLY_FILLED","PARTIALLY_FILLED_PART")
                    ...
                    if str(data.get("e","")).upper() == "ORDER_TRADE_UPDATE":
                        ...
                        if _is_reduce_only_tp(o):
                            ...
                            try:
                                ...
                                logger.warning({"event":"tp_handler_error","symbol":sym,"err":str(e)})
...
async def stop_user_stream_consumer():
    ...
    _running = False
    try:
        if _ws_task and not _ws_task.done(): _ws_task.cancel()
