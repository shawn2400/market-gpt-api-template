...
def enforce(inputs: SopInputs) -> Dict[str, Any]:
    ...
    if get_flag(FEAT_BTC_GATE, False) and inputs.btc_trend_ok is False:
        reasons.append(_rc("btc_trend_mismatch"))
    if get_flag(FEAT_TF_ALIGN, False) and inputs.tf_aligned is False:
        reasons.append(_rc("tf_not_aligned"))
    if (get_flag(FEAT_TF_ALIGN, False) or get_flag(FEAT_BTC_GATE, False)) and inputs.sym_trend_ok is False:
    ...
    return {
        "ok": ok,
        "reason_code": (None if ok else reasons[0]),
        "reasons": reasons,
