...
# ברירת מחדל למצב מרג'ין – נתמך גם דרך ENV וגם דרך config.py
_BINANCE_MARGIN_TYPE_DEFAULT = str(
    getattr(config, "BINANCE_MARGIN_TYPE_DEFAULT", os.getenv("BINANCE_MARGIN_TYPE_DEFAULT", "ISOLATED"))
).strip().upper()
if _BINANCE_MARGIN_TYPE_DEFAULT not in ("ISOLATED", "CROSSED", "CROSS"):
    _BINANCE_MARGIN_TYPE_DEFAULT = "ISOLATED"
...
# ---- NEW: Margin Type helpers (ISOLATED/CROSSED) ----
def _normalize_margin_type(mt: Optional[str]) -> str:
    m = (mt or "").strip().upper()
    if m in ("ISOLATED",):
        return "ISOLATED"
    if m in ("CROSSED", "CROSS"):
        return "CROSSED"
    return _BINANCE_MARGIN_TYPE_DEFAULT if _BINANCE_MARGIN_TYPE_DEFAULT in ("ISOLATED", "CROSSED") else "ISOLATED"


def _read_margin_type(client, symbol: str) -> Optional[str]:
    """
    מחזיר "ISOLATED"/"CROSSED" אם הצליח לקרוא; אחרת None.
    """
    try:
        info = retry_call(lambda: client.futures_position_information(symbol=symbol.upper()),
                          f"futures_position_information({symbol})")
        if isinstance(info, list) and info:
            mt = info[0].get("marginType", "")  # 'isolated'/'cross'
            if isinstance(mt, str):
                mt_u = mt.strip().upper()
                if mt_u in ("ISOLATED", "CROSS"):
                    return "ISOLATED" if mt_u == "ISOLATED" else "CROSSED"
    except Exception as e:
        logging.warning(f"[TRADER] read margin type failed: {e}")
    return None


def _ensure_margin_type(client, symbol: str, want_mt: str) -> bool:
    """
    מנסה להגדיר מצב מרג'ין. אם יש פוזיציה פתוחה, הבורסה עלולה להחזיר שגיאה – נתעד ונמשיך.
    """
    try:
        want = _normalize_margin_type(want_mt)
        cur = _read_margin_type(client, symbol)
        if cur == want:
            return True
        if SKIP_MUTATIONS:
            logging.warning("[TRADER] margin type mismatch (cur=%s, want=%s) but mutations disabled.", cur, want)
            return False
        ...
        logging.info("[TRADER] margin_type set %s -> %s", symbol, resp)
        return True
    except Exception as e:
        # למשל: "Margin type can only be changed when there is no position." – נתעד ונמשיך
...
async def binance_futures_trade(
    ...
    margin_type: Optional[str] = None,  # <--- NEW
) -> Dict[str, Any]:
    ...
    want_mt = _normalize_margin_type(margin_type)
    await asyncio.to_thread(_ensure_margin_type, client, symbol, want_mt)
    ...
    logging.info(
        ...
        symbol.upper(), side, entry_s, sl_s, tp_s, qty_s, tick_size, step_size, min_qty, min_notional, lev, budget, notional, want_mt
    )
    ...
    return {
        ...
        "tp": float(tp_dec),
        "leverage": int(lev),
        "marginType": want_mt,  # <--- NEW: נחזיר מה הופעל בפועל
