...
async def _safe_analyze(symbol: str, tf: str, market: str, trending_only: bool) -> Optional[Dict]:
    try:
        return await analyze_symbol(
            symbol=symbol, market_type=market, interval=tf, limit=150,
            trending_only=trending_only, with_ai=False, frames=[tf],
        )
    except Exception as e:
...
async def multi_tf_scan_with_ai(
    ...
    market = str(market if (market := (markets[0] if markets else "futures")) else "futures").lower()
    ...
    for r in results_raw:
        if isinstance(r, Exception):
            logging.error(f"[multi_tf_scanner] analyze task exception: {r}", exc_info=True); continue
        if r and isinstance(r, dict) and r.get("symbol"):
            frames.append(r)

    if not frames:
        logging.info("[multi_tf_scanner] No frame results."); return []
    ...
    for sym, data in grouped.items():
        ...
        def _fallback() -> Dict:
            ...
            return {
                "symbol": sym, "direction": direction, "quality_score": round(avg_q, 2),
                "signal": "BUY" if direction == "LONG" else "SELL",
                "confidence": 50.0, "frames": frames_list, "details": data,
            }
