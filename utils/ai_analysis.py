...
async def analyze_with_ai(data: dict) -> str:
    ...
    try:
        ...
        logger.warning("⚠️ analyze_with_ai fallback due to error: %s", e)
...
def _parse_sltp(text: str) -> Optional[Tuple[float, float]]:
    ...
    if "SL=" in t and "TP=" in t:
        try:
            ...
            logger.warning("⚠️ Failed to parse SL/TP: %s", e)
            return None
    return None
...
async def predict_optimal_sl_tp(symbol: str, direction: str, entry_price: float, atr: float | None = None) -> tuple[float, float]:
    ...
    try:
        ...
        logger.warning("⚠️ predict_optimal_sl_tp fallback due to error: %s", e)

    # פולבק: נגזרות אחוזיות בטוחות
    long = str(direction).upper() == "LONG"
    sl_pct = _clip_pct(0.60, cfg.SL_MIN_PCT, cfg.SL_MAX_PCT)  # 0.60% כברירת מחדל
    tp_pct = _clip_pct(2.00, cfg.TP_MIN_PCT, cfg.TP_MAX_PCT)  # 2.00% כברירת מחדל
