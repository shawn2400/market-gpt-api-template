...
def get_symbol_step(symbol, futures=False):
    ...
    מחזיר dict: {'price_step', 'qty_step'}
    """
    info = client.futures_exchange_info() if futures else client.get_exchange_info()
    for s in info['symbols']:
        if s['symbol'] == symbol:
            price_step = qty_step = 0.01
            for f in s['filters']:
                if f['filterType'] == 'PRICE_FILTER':
                    price_step = float(f['tickSize'])
                if f['filterType'] == 'LOT_SIZE':
                    qty_step = float(f['stepSize'])
            return {"price_step": price_step, "qty_step": qty_step}
    return {"price_step": 0.01, "qty_step": 0.01}

def create_grid_levels(price, price_step, grid_count, grid_pct=0.5, direction="BOTH"):
    """ יוצר רמות גריד מעוגלות לפי step החוקי """
    levels = []
    for i in range(1, grid_count + 1):
        up = round_step(price * (1 + grid_pct / 100 * i), price_step)
        down = round_step(price * (1 - grid_pct / 100 * i), price_step)
...
def execute_grid(symbol, budget=100, grid_count=6, grid_pct=0.5, 
    ...
    levels = create_grid_levels(price, price_step, grid_count, grid_pct=grid_pct, direction=direction)
    open_orders = []
    for lvl in levels:
