...
try:
    ...
    logging.info("[GRID] dedicated binance_grid_trade not available: %s (DRY-run)", _e)
...
def _validate_inputs(symbol: str, budget_usd: float, grid_count: int,
                     grid_pct: float, tp_pct: float, sl_pct: float,
                     leverage: int, futures: bool) -> Optional[str]:
...
def _mk_tp_sl(base: float, side: str, tp_pct: float, sl_pct: float) -> Tuple[float, float]:
    tpm, slm = tp_pct / 100.0, sl_pct / 100.0
    if side.upper() == "BUY":
        tp, sl = base * (1.0 + tpm), base * (1.0 - slm)
    else:
        tp, sl = base * (1.0 - tpm), base * (1.0 + slm)
    return round(tp, 6), round(sl, 6)


def build_grid_plan(*, symbol: str, live_price: float, budget_usd: float,
                    grid_count: int, grid_pct: float, tp_pct: float,
                    sl_pct: float, futures: bool, leverage: int) -> Dict[str, Any]:
    ...
    for _ in range(half):
        ...
        levels.append({"side": "BUY", "price": round(p_down, 6), "tp": tp_px, "sl": sl_px, "budget": round(tranche, 4)})
    ...
    for _ in range(grid_count - half):
        ...
        levels.append({"side": "SELL", "price": round(p_up, 6), "tp": tp_px, "sl": sl_px, "budget": round(tranche, 4)})
...
async def execute_grid_trade(*, symbol: str, budget_usd: float,
                             grid_count: int = 6, grid_pct: float = 0.4,
                             leverage: int = 20, futures: bool = True,
                             tp_pct: float = 1.5, sl_pct: float = 1.0) -> Dict[str, Any]:
    try:
        ...
        plan = build_grid_plan(symbol=symbol, live_price=float(live), budget_usd=budget_usd,
                               grid_count=grid_count, grid_pct=grid_pct,
                               tp_pct=tp_pct, sl_pct=sl_pct,
                               futures=futures, leverage=leverage)

        if MUTATIONS_DISABLED or not _BINANCE_GRID_OK:
            reasons = []
            if MUTATIONS_DISABLED:
                if not EXECUTE_TRADES: reasons.append("EXECUTE_TRADES=false")
                if BINANCE_SKIP_ACCOUNT_MUTATIONS_ENV: reasons.append("BINANCE_SKIP_ACCOUNT_MUTATIONS=true")
            if not _BINANCE_GRID_OK: reasons.append("binance_grid_trade unavailable")
            return {"status": "dry_run", "reason": " & ".join(reasons), "plan": plan}

        result = await binance_grid_trade(
            symbol=plan["symbol"], market_type=plan["market_type"], leverage=plan["leverage"],
            levels=plan["levels"], total_budget=plan["total_budget"],
        )
