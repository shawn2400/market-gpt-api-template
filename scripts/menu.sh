...
quick_status() {
  ...
  if curl -s -H "Authorization: Bearer $BEARER" "$BASE/telegram/status" | grep -q '"ok":true'; then
    tg_ok="OK"
    tg_display="${G}🧠 Telegram OK${N}"
  else
    tg_ok="DOWN"
    tg_display="${R}⚠️ Telegram Off${N}"
  fi

  http_code=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/readyz" || echo "000")
  if [ "$http_code" = "200" ]; then
    render_ok="OK"
    render_display="${G}☁️ Render OK${N}"
  else
    render_ok="DOWN"
    render_display="${R}☁️ Render Down${N}"
  fi
  ...
  # 🔔 שלח נוטיפיקציה רק אם יש שינוי במצב
